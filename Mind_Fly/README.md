...
GRALP-MindSpore 是基于**昇思MindSpore**框架实现的轻量级PPO局部规划器，面向完全随机化、无地图的自主运动场景，通过向量化的广义射线/深度离障距离+运动学历史作为观测，输出连续的平面速度控制指令。本项目全流程基于MindSpore 2.8开发，适配Ascend/GPU/CPU硬件平台，支持批量仿真训练、混合精度加速，并提供轻量化的推理部署能力，可直接对接实际运动控制设备。

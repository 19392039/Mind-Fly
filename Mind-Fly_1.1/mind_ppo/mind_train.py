...
def main():
    ...
    while global_step < total_env_steps:
        ...
        if global_step - last_log >= log_interval:
            ...
            def to_tensor(x):
                return ms.Tensor(float(x), dtype=ms.float32)

            summary_record.add_value('scalar', "loss/policy", to_tensor(avg_policy_loss))
            summary_record.add_value('scalar', "loss/value", to_tensor(avg_value_loss))
            summary_record.add_value('scalar', "loss/total", to_tensor(avg_total_loss))
            summary_record.add_value('scalar', "metric/entropy", to_tensor(avg_entropy))
            summary_record.add_value('scalar', "metric/approx_kl", to_tensor(avg_approx_kl))
            summary_record.add_value('scalar', "speed/fps", to_tensor(fps))

            if finished_rewards:
                avg_ep_rew = np.mean(finished_rewards)
                summary_record.add_value('scalar', "episode/avg_reward", to_tensor(avg_ep_rew))
